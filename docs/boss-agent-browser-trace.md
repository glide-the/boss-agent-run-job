...
采集 `agent-browser` 在 BOSS 直聘中从 chat 对话页进入具体岗位详情页的自动化轨迹，明确每一步的页面区域、点击方式、信息区域和采集方法，并沉淀为可复用脚本。
...
| 岗位描述 | 详情正文 | 优先 selector，其次从 raw text 后处理 | 后续可扩展 |
| 技能关键词 | 岗位描述/标签区 | 标签 selector 或关键词抽取 | 后续可扩展 |
| 公司名称 | 公司信息区 | selector 或包含公司后缀的短行 | `company` |
| 公司规模 | 公司信息区 | 匹配人数规模文本 | 后续可扩展 |
| 行业 | 公司信息区 | selector 或行业词表匹配 | 后续可扩展 |
...
4. 页面文本读取  
   使用 `agent-browser read` 保存详情页可见文本，再用规则抽取字段。原始文本保留在 `output/raw/`，结构化结果保留在 `output/jobs.json`。

5. 截图校验  
   使用 `agent-browser screenshot` 保存岗位详情页截图，方便检查脚本点击是否进入了正确页面。

6. 轨迹事件记录  
   每一步写入 `output/trace-events.json`，包括打开页面、目标 `target_id`、尝试 locator、点击成功、采集完成、失败原因。
...
- `output/chats.json`：结构化聊天数据，包含 `target_id`
- `output/jobs.json`：结构化岗位数据，包含 `target_id` 与 URL 派生的 `job_id`
